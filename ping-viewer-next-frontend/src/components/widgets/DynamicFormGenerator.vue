    ...
    <div class="max-w-3xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      ...
      <div class="p-6">
        ...
        <div v-if="generatedCommand" class="mt-8">
          <h3 class="text-lg font-medium text-gray-900 mb-4">Generated Command:</h3>
          <pre class="bg-gray-100 p-4 rounded-md overflow-x-auto messages-container h-64 overflow-y-auto mb-4">{{ JSON.stringify(generatedCommand, null, 2) }}</pre>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async fetchAvailableDevices() {
      try {
        const response = await fetch(`${window.location.origin}/device_manager/List`);
    ...
    generateCommand() {
      ...
      this.$emit('command-generated', command);
    },
