	  ...
	  `;
		...
		function updateTexture() {
			...
			measurementHistory.value.forEach((measurement, columnIndex) => {
				...
				const scaleRatio =
					(measurement.maxDepth - props.minDepth) /
				...
				for (let y = 0; y < newHeight; y++) {
					...
					if (normalizedY < scaleRatio) {
						...
						if (dataIndex < dataLength) {
							...
							const color = props.getColorFromPalette(
								value,
								props.colorPalette,
							);
			...
			if (newData.length > 0) {
				...
				if (measurementHistory.value.length > 0) {
					const maxHistoricalDepth = Math.max(
						...measurementHistory.value.map((m) => m.maxDepth),
					);
		...
		function render() {
			...
			const virtualMaxDepthLocation = gl.getUniformLocation(
				shaderProgram,
				"u_virtualMaxDepth",
			);
			gl.uniform1f(virtualMaxDepthLocation, virtualMaxDepth.value);

			const minDepthLocation = gl.getUniformLocation(
				shaderProgram,
				"u_minDepth",
			);
