...
const connectWebSocket = () => {
	...
	socket.value.onopen = () => {
		console.log("Ping1D WebSocket connected");
	};
	...
	socket.value.onmessage = (event) => {
		try {
			...
			if (profile) {
				...
				if (props.debug) {
					console.log("Processed Ping1D data:", newData);
				}
			}
		} catch (error) {
	...
	socket.value.onclose = () => {
		console.log("Ping1D WebSocket disconnected");
		socket.value = null;
...
onMounted(() => {
	console.log("Ping1DLoader mounted with props:", props);
	connectWebSocket();
});
