...
const rotateTextureData = (lineOffset: number) => {
	...
	for (let i = 0; i < props.numLines; i++) {
		...
		textureData.value.set(
			tempData.slice(sourceStart, sourceStart + bytesPerLine),
			destStart,
		);
	}
};
...
const render = () => {
	...
	gl.uniform1f(
		gl.getUniformLocation(shaderProgram, "uStartAngle"),
		props.startAngle,
	);
	gl.uniform1f(
		gl.getUniformLocation(shaderProgram, "uEndAngle"),
		props.endAngle,
	);
