<template>
  <div class="bg-gray-100 min-h-screen p-6">
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      ...
      <div class="p-6">
        <button @click="redirectToDocs" class="mb-6 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">
        ...
        <div ref="messagesContainer" class="messages-container bg-gray-100 p-4 rounded h-64 overflow-y-auto mb-4">
          <div v-for="(message, index) in messages" :key="index" class="mb-2">{{ message }}</div>
        </div>
        ...
        <div class="flex items-center mb-4">
          ...
          />
          <button @click="sendMessage" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded">
            Send
          </button>
        </div>
