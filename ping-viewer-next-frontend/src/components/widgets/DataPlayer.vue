...
const play = () => {
	...
	startTime =
		performance.now() -
		(new Date(loadedData.value[currentFrame.value].timestamp).getTime() -
			baseTimestamp);
	playNextFrame();
};
...
const playNextFrame = () => {
	...
	if (currentFrame.value < loadedData.value.length) {
		...
		const timeToNextFrame = Math.max(
			0,
			targetElapsedTime - (currentTime - startTime),
		);
...
const updateCurrentFrame = () => {
	currentFrame.value = Math.min(
		Math.max(0, currentFrame.value),
		loadedData.value.length - 1,
	);
