			...
			<template v-else>
				<v-app-bar class="flex-none">
					...
					<v-tooltip location="bottom">
						<template v-slot:activator="{ props }">
							<v-icon v-bind="props" :color="yawConnectionStatus === 'Connected' ? 'success' : 'error'"
								class="mr-2">
								mdi-compass
							</v-icon>
						</template>
					...
					<v-menu location="bottom end">
						...
						<v-card min-width="300" class="recordings-menu">
							<v-card-title class="d-flex align-center py-2">
								...
								<v-spacer></v-spacer>
								<v-btn v-if="recordings.length" density="comfortable" variant="text"
									@click="selectView('Replay')">
									Open Replay View
								</v-btn>
							</v-card-title>
							...
							<v-list v-if="recordings.length" class="recordings-list">
								<v-list-item v-for="recording in recordings" :key="recording.id"
									...
									<template v-slot:prepend>
										<v-icon
											:icon="recording.deviceType === 'Ping360' ? 'mdi-radar' : 'mdi-altimeter'"></v-icon>
									</template>
									...
									<template v-slot:append>
										<div class="d-flex gap-2">
											<v-btn icon="mdi-play" variant="text" size="small"
												@click="playRecording(recording)" v-if="recording.downloaded"></v-btn>
						...
						<v-menu location="bottom end">
							...
							<v-list>
								...
								<v-list-item @click="toggleFullscreen">
									...
									<v-list-item-title>{{ isFullscreen ? 'Exit Fullscreen' : 'Fullscreen'
										}}</v-list-item-title>
								</v-list-item>
							</v-list>
						</v-menu>
				...
				<v-main class="flex-1">
					<div class="h-full overflow-auto">
						<component :is="views[selectedView]" :serverUrl="serverUrl" :websocket="websocket"
							:websocketStatus="websocketStatus" :deviceData="deviceData"
							@send-message="sendWebSocketMessage" class="h-full w-full" />
					</div>
				</v-main>

				<v-dialog v-model="showSettings" max-width="600px">
					<v-card>
						<v-card-title class="text-h5 pb-2">Settings</v-card-title>
						...
						<v-card-text>
							<VisualSettings :common-settings="commonSettings" :ping1DSettings="ping1DSettings"
								...
								@update:ping1D-settings="updatePing1DSettings"
								@update:ping360-settings="updatePing360Settings" @update:is-dark-mode="updateDarkMode"
								@save="saveSettings" @reset="resetSettings" />
						</v-card-text>

						<v-card-actions>
							<v-spacer></v-spacer>
							<v-btn color="grey" text @click="showSettings = false">Close</v-btn>
						</v-card-actions>
					</v-card>
				</v-dialog>
			</template>
...
// Check if current route is a widget route
