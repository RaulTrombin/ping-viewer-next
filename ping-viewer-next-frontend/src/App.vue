...
const connectYawWebSocket = (url) => {
	if (yawWebSocket?.readyState === WebSocket.OPEN) {
		console.log("Yaw WebSocket is already connected");
		return;
	}
	...
	try {
		...
		yawWebSocket.onopen = () => {
			console.log("Yaw WebSocket connected");
		...
		yawWebSocket.onclose = () => {
			console.log("Yaw WebSocket disconnected");
	...
	try {
		...
		if (commonSettings.customPalette?.length > 0) {
			localStorage.setItem(
				"customColorPalette",
				JSON.stringify(commonSettings.customPalette),
			);
		}
...
const processWebSocketMessage = (data) => {
	...
	if (data.DeviceMessage) {
		...
		deviceData[deviceId][messageType] =
			data.DeviceMessage.PingMessage[messageType];
	}
};
