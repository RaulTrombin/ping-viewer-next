...
impl DeviceManager {
    pub async fn firmware_update(
        ...
        let firmware_path = match selected_device_updagrade {
    ...
    pub async fn change_device_status(
        ...
        if let Some(device) = self.device.get_mut(&device_id) {
            ...
            // Re-evaluate discovery service state after a status change
            self.sync_discovery_service();
            // Keep discovery skip list in sync immediately after status change
            if let Ok(Answer::DeviceInfo(inner)) = self.list().await {
    ...
    fn discovery_should_run(&self) -> bool {
        !self.device.values().any(|d| {
            matches!(
                d.status,
                DeviceStatus::Bootloader | DeviceStatus::FirmwareUpdateInProgress
            )
        })
    }
    ...
    pub async fn register_device(
        ...
        if self.device.contains_key(&id) {
            ...
            if let Some(existing) = self.device.get_mut(&id) {
                if existing.status == DeviceStatus::Bootloader {
                    ...
                    // Discovery state may change as device leaves bootloader
                    self.sync_discovery_service();
        ...
        // Ensure discovery state matches current fleet
        self.sync_discovery_service();
