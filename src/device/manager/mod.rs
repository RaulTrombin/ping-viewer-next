...
pub struct Device {
    ...
    pub firmware_update_handle: Option<tokio::task::JoinHandle<()>>,
}
...
pub enum Request {
    ...
    #[serde(skip)]
    ChangeDeviceStatus(ChangeDeviceStatusRequest),
}
...
impl DeviceManager {
    pub async fn firmware_update(
        ...
        let (source, device_id_opt, revision_opt, status_opt, device_type_opt, _manual_path_opt) =
            match request.mode {
                FirmwareUpdateMode::AutoUpdate(ref uuidw) => {
                    ...
                    selected_device_upgrade = device_kind;
                ...
                FirmwareUpdateMode::ManualUpdate(ManualUpdate {
                    ...
                    selected_device_upgrade = device_kind.to_owned();
                    ...
                    let is_update_in_progress = self.device.values().any(|device| {
                        matches!(device.status, DeviceStatus::Updating) &&
        ...
        let firmware_path = match selected_device_upgrade {
                            ...
                            DeviceStatus::Running | DeviceStatus::ContinuousMode => {
                                    ...
                                    self.get_device_handler(device_id).await
                                {
                                    ...
                                    if let Err(err) = handler
                                        .send(crate::device::devices::PingRequest::Ping1D(
                                            crate::device::devices::Ping1DRequest::ContinuousStop(
                                                bluerobotics_ping::ping1d::ContinuousStopStruct {
                                                    id,
                                                },
                                            ),
                                        ))
                                    ...
                                    if let Err(err) = handler
                                        ...
                                        .await
                                    {
                                        error!("Something went wrong while executing bootloader routine, details: {err:?}, device: {device_id}");
                                    }
            ...
            tokio::spawn(async move {
                if let Some(device_id) = device_id {
                    ...
                    let _ = manager_handler
                        .send(Request::ChangeDeviceStatus(ChangeDeviceStatusRequest {
                            device_id,
                            status: new_status,
                        }))
                        .await;
        ...
        let update_result = match selected_device_upgrade {
            FirmwareDeviceKind::Ping1D | FirmwareDeviceKind::Ping2 => {
        ...
        let res = match update_result {
            ...
            Err(_e) => {
                // Ensure discovery state is consistent even on error
                self.sync_discovery_service();
                return Err(ManagerError::FirmwareUpdateError(
                    FirmwareUpdateError::UnsupportedDevice,
                ));
            }
    ...
    pub async fn change_device_status(
        &mut self,
        device_id: Uuid,
        status: DeviceStatus,
    ) -> Result<Answer, ManagerError> {
        if let Some(device) = self.device.get_mut(&device_id) {
            info!(
                "DeviceManager: Device {} changed status to {:?}",
                device_id, status
            );
            ...
            if let Ok(Answer::DeviceInfo(inner)) = self.list().await {
                ...
                Ok(Answer::DeviceInfo(inner))
            } else {
                Err(ManagerError::Other(
                    "Failed to list devices after status change".to_string(),
                ))
            }
        } else {
    ...
    async fn handle_message(&mut self, actor_request: ManagerActorRequest) {
        ...
        match actor_request.request {
            ...
            Request::ChangeDeviceStatus(req) => {
                let answer = self.change_device_status(req.device_id, req.status).await;
                if let Err(err) = actor_request.respond_to.send(answer) {
                    error!("DeviceManager: Failed to return ChangeDeviceStatus response: {err:?}");
                }
            }
            _ => {
    ...
    fn discovery_should_run(&self) -> bool {
        !self
            .device
            .values()
            .any(|d| matches!(d.status, DeviceStatus::Bootloader | DeviceStatus::Updating))
    }

    fn sync_discovery_service(&mut self) {
        if self.discovery_should_run() {
