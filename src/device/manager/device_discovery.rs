...
pub async fn blueos_ping_discovery() -> Option<BluePingDiscoveryResult> {
    // Make HTTP request to Blue Robotics Ping service
    ...
    for device in devices {
        // Track serial ports that are being handled via UDP
        if !device.port.is_empty() {
            used_ports.push(device.port);
        }

        // If device has a UDP port, add it
        ...
        else if let Some(ethernet_info) = device.ethernet_discovery_info {
            if let Some((ip, port)) = ethernet_info.split_once(':') {
                if let (Ok(ip), Ok(port)) = (ip.parse::<Ipv4Addr>(), port.parse::<u16>()) {
                    available_sources.push(SourceSelection::UdpStream(SourceUdpStruct {
                        ip,
                        port,
                    }));
...
pub async fn blue_ping_discovery() -> Option<Vec<SourceSelection>> {
    None
}
...
pub async fn serial_discovery(skip_ports: Option<&[String]>) -> Option<Vec<SourceSelection>> {
    match available_ports() {
        Ok(serial_ports) => {
            ...
            let filtered_ports = serial_ports.into_iter().filter(|port_info| {
                if let Some(skip_ports) = skip_ports {
                    !skip_ports.contains(&port_info.port_name)
                } else {
                    true
                }
            });
